...
# things seem wired here and I think perhaps we can explain the reason rather than do the i.i.d test.
...
# It seems that NDAI has a high correlation with expert labels and radiances have high correlations wiht each other.
View(cor(data2))
View(cor(data3))
...
ggplot(train)+
  ...
  facet_wrap(~expert_label)+
  geom_vline(xintercept = 0.195, color = 'red')
...
CVgeneric <- function(classifier, features, labels, k, loss.function){
  ...
  return(list(error.rate = result / k))
} 
# test
...
svm.fit <- svm(expert_label~NDAI+SD+CORR, data = cbind(labels, features))
...
# decision tree
rpart.fit <- rpart(as.formula(paste(names[1], paste(names[-1], collapse = '+'), sep = " ~ ")), data = tra, , method = "class")
rpart.pred <- predict(rpart.fit, val, ,type="prob")
